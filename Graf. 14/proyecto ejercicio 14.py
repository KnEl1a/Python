...
archivo = 'chart (14).csv'
datos = pd.read_csv(archivo, usecols=['ANIOS','Exportaciones','Importaciones','Balanza']) 
df = pd.DataFrame(datos, columns=['ANIOS','Exportaciones','Importaciones','Balanza']) # no reconoce la ñ y lo que son las tildes de nuestro 
...
plt.bar(df['ANIOS'], df['Exportaciones'], width=1/5, label = "Exportaciones", color= "red")
plt.bar(df['ANIOS'] + 0.2, df['Importaciones'], width=1/5, label = "Importaciones")
plt.bar(df['ANIOS'] + 0.4, df['Balanza'], width=1/5, label = "Balanza")
